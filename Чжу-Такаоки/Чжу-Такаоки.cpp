...
void preBmBc(string, int bmBc[ALPHABET_SIZE][ALPHABET_SIZE]);
...
int BoyerMooreStringSearch(string str, string pattern)
{
	...
	int bmBc[ALPHABET_SIZE][ALPHABET_SIZE];
	int* bmGs = new int[patternLength];
	preBmBc(pattern, bmBc);
	preBmGs(pattern, bmGs);
	for (int i = patternLength - 1; i < strLength; i += Max(bmBc[str[j - (j > 1)]][str[j]] - (i - j), bmGs[patternLength - (i - j) - 1]))
	{
...
void preBmBc(string pattern, int bmBc[ALPHABET_SIZE][ALPHABET_SIZE])
{
	...
	for (int i = 0; i < ALPHABET_SIZE; ++i)
		for (int j = 0; j < ALPHABET_SIZE; ++j)
			bmBc[i][j] = length;
	for (int i = 0; i < ALPHABET_SIZE; ++i)
		bmBc[i][pattern[0]] = length - 1;
	for (int i = 1; i < length - 1; ++i)
		bmBc[pattern[i - 1]][pattern[i]] = length - i - 1;
}
